...
module.exports.run = async (bot, message, args) => {
        ...
        try {
            ...
            while (cevap.length !== confirmation.length && point < 6) {
                await message.channel.send(new Discord.RichEmbed().setColor('BLACK').setDescription(stripIndents`
                    ${displayText === null ? '**Menelaos Adam Asmaca**!' : displayText ? '**Çok iyisin!**' : '**Yanlış Harf!**'}
                         **Kelime:**    \`${display.join(' ')}\`
                ...
                if (!guess.size) {
                    await message.channel.send(new Discord.RichEmbed().setColor('BLACK').setDescription('Zamanın doldu!'));
                    break;
                }
            ...
            if (cevap.length === confirmation.length || tahmin) return message.channel.send(new Discord.RichEmbed().setColor('BLACK').setDescription(`**Tebrikler kelimeyi buldun! **${cevap}!`));
            return message.channel.send(new Discord.RichEmbed().setColor('BLACK').setDescription(`Maalesef bilemedin kelime bu: **${cevap}**`));
        } catch (err) {
            oyndurum.delete(message.channel.id);
